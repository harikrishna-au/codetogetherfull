...
router.get('/stats/:userId', asyncHandler(async (req: Request, res: Response) => {
    ...
    const questionIds = solvedProblems?.map((i: any) => i.question_id).filter(Boolean) || [];

    const { data: questions } = questionIds.length > 0
        ? await supabase.from('questions').select('id, difficulty').in('id', questionIds)
        : { data: [] };

    const difficultyMap = new Map(questions?.map((q: any) => [q.id, q.difficulty]) || []);
    ...
    languageStats?.forEach((item: any) => {
    ...
    const totalCodingTime = durations?.reduce((sum: number, item: any) => sum + (item.duration || 0), 0) || 0;
...
router.get('/history/:userId', asyncHandler(async (req: Request, res: Response) => {
    ...
    const partnerMap = new Map(partnersResponse.data?.map((p: any) => [p.user_id, p]) || []);
    const questionMap = new Map(questionsResponse.data?.map((q: any) => [q.id, q]) || []);
