...
import { Question as IQuestion, Example, TestCase } from '@/types/index.js';
...
questionSchema.methods.updateAverageCompletionTime = function(newTime: number) {
...
questionSchema.statics.getRandomQuestion = function(difficulty: string, excludeIds: string[] = []) {
...
questionSchema.statics.getMostUsedQuestions = function(limit: number = 10) {
  return this.find({})
    .sort({ usageCount: -1 })
    .limit(limit)
